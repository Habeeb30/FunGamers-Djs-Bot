  ...
  async execute(interaction) {
    ...
    if (!target.voice.channel) {
      return interaction.reply({
        content: `**The member was not found in the voice channel.**`,
        ephemeral: true,
      });
    }
    ...
    if (member.voice.channel.members.size == 2) {
      return interaction.reply({
        content: `**You cannot vote in a channel with only 2 members.**`,
        ephemeral: true,
      });
    }
    ...
    const message = await interaction.reply({
      ...
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            `**starts voting to kick <@${target.id}> out**\n\n` +
              `**Time to vote:** _1 minutes..._\n` +
              `**Need votes**: _${votesNeed}_`
          ),
      ],
      fetchReply: true,
    });
    ...
    message
      .awaitReactions({ filterReactions, maxUsers: users, time: 1000 * 60 * 1 })
      .then((collected) => {
            ...
            .edit({
              ...
              embeds: [
                new EmbedBuilder()
                  ...
                  .setThumbnail(target.displayAvatarURL())
                  .setDescription(
                    `**Voting is over - <@${target.id}> is kiked out.**\n\n` +
